...
module.exports={
    ...
    register: async (req,res)=>{
        ...
        if(userFound) return res.status(409).send("Username is already Taken :(")
    ...
    login: async (req,res)=>{
        ...
        const db= req.app.get('db')
        const [userFound]= await db.user.find_user_by_username(username)
        if(!userFound)return res.status(404).send('Cant find that User!')       
        const isAuth= bcrypt.compareSync(password, userFound.password)
        if(!isAuth) return res.status(403).send('Not the right password')        
